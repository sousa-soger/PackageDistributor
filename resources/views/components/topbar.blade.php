    ...
    <div class="flex items-center gap-4 px-6 lg:px-8 h-16">
        ...
        <div x-data="{
            ...
            <button @click="toggle()" type="button"
                ...
                <svg x-show="theme === 'light'" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                    stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <!--
        {{-- Notifications --}}
        <button type="button" class="relative h-9 w-9 flex items-center justify-center rounded-lg transition-colors"
            style="color: hsl(var(--muted-foreground));" x-bind:class="'hover:bg-[hsl(var(--secondary))]'"
        ...
        <div x-data="{ open: false }" class="relative" @keydown.escape.window="open = false">
            <button @click="open = !open" type="button" aria-label="Open profile menu" :aria-expanded="open"
                ...
                <span
                    class="relative flex shrink-0 overflow-hidden rounded-full h-9 w-9 ring-2 transition-all duration-300 ease-out hover:ring-4 hover:ring-offset-2 hover:ring-offset-background hover:ring-[hsl(var(--primary)/0.35)] cursor-pointer"
            ...
            <div x-show="open" x-cloak @click.outside="open = false"
                ...
                <div class="px-2 py-1.5 text-sm font-semibold">
                    <div class="flex flex-col">
                        <span class="text-sm font-semibold"
                            style="color:hsl(var(--foreground))">{{ Auth::user()->name }}</span>
                        <span class="text-xs font-normal"
                            style="color:hsl(var(--muted-foreground))">{{ Auth::user()->email }}</span>
                    </div>
                </div>
                ...
                {{-- Quick connect --}}
                <div class="px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wider"
                    style="color:hsl(var(--muted-foreground))">Quick connect</div>
                ...
                @if(Auth::user()->github_id)
                    <form method="POST" action="{{ route('github.oauth.disconnect') }}">
                        ...
                        <button type="submit" role="menuitem" tabindex="-1"
                            class="w-full relative flex cursor-pointer select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground"
                            style="color:hsl(var(--foreground))" aria-label="Disconnect GitHub">
                            <span
                                class="flex h-6 w-6 shrink-0 items-center justify-center rounded-md border border-border/60 bg-secondary/60 text-muted-foreground">
                            ...
                            <span
                                class="inline-flex shrink-0 items-center gap-1.5 rounded-md border border-success/30 bg-success/10 px-2 py-0.5 text-[10px] font-medium text-success">
                ...
                @else
                    <a href="{{ route('github.oauth.redirect') }}" role="menuitem" tabindex="-1"
                        class="relative flex cursor-pointer select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground"
                        style="color:hsl(var(--foreground))">
                        <span
                            class="flex h-6 w-6 shrink-0 items-center justify-center rounded-md border border-border/60 bg-secondary/60 text-muted-foreground">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        ...
                        <span
                            class="inline-flex shrink-0 items-center gap-1.5 rounded-md border border-border/60 bg-muted/60 px-2 py-0.5 text-[10px] font-medium text-muted-foreground">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                class="h-3 w-3">
                ...
                {{-- GitLab quick connect --}}
                @if(Auth::user()->gitlab_id)
                    <form method="POST" action="{{ route('gitlab.oauth.disconnect') }}">
                        @csrf
                        <button type="submit" role="menuitem" tabindex="-1"
                            class="w-full relative flex cursor-pointer select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground"
                            style="color:hsl(var(--foreground))" aria-label="Disconnect GitLab">
                            <span
                                class="flex h-6 w-6 shrink-0 items-center justify-center rounded-md border border-border/60 bg-secondary/60 text-muted-foreground">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            ...
                            <span class="flex-1 text-left">GitLab</span>
                            <span
                                class="inline-flex shrink-0 items-center gap-1.5 rounded-md border border-success/30 bg-success/10 px-2 py-0.5 text-[10px] font-medium text-success">
                                <span class="h-1.5 w-1.5 rounded-full bg-current"></span>
                                Connected
                ...
                @else
                    <a href="{{ route('gitlab.oauth.redirect') }}" role="menuitem" tabindex="-1"
                        class="relative flex cursor-pointer select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground"
                        style="color:hsl(var(--foreground))">
                        <span
                            class="flex h-6 w-6 shrink-0 items-center justify-center rounded-md border border-border/60 bg-secondary/60 text-muted-foreground">
                ...
                {{-- Settings --}}
                <a role="menuitem" tabindex="-1" href="{{ route('settings') }}"
                    class="relative flex cursor-pointer select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-none transition-colors hover:bg-accent"
                    style="color:hsl(var(--foreground))">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
