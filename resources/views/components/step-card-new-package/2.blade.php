            ...
            <div class="grid grid-cols-1 items-stretch gap-3 lg:grid-cols-[1fr_auto_1fr]">
                <div class="min-w-0 space-y-2">
                    <p
                        ...
                        x-cloak
                        :class="selectedVersionBase ? 'visible' : 'invisible'" {{-- so that list doesn't move down when selected --}}
                    >
                        Selected:
                        <span class="font-semibold" x-text="selectedBaseLabel"></span>
                        <span class="text-slate-500">(Base)</span>
                    </p>
                    <div class="max-h-[min(420px,55vh)] overflow-y-auto overflow-x-hidden rounded-xl border border-slate-200 bg-white">
                                ...
                                <div class="flex min-w-0 items-start gap-3">
                                    ...
                                    <div class="min-w-0 space-y-0.5">
                                        <div class="flex flex-wrap items-center gap-2">
                                            ...
                                            >DEFAULT</span>
                                            <span
                                                x-show="version.type === 'release'"
                                                class="rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-semibold uppercase text-blue-700"
                                            >Release</span>
                                            <span
                        ...
                        <div
                            x-show="filteredVersionsBase.length === 0"
                            class="px-4 py-8 text-sm text-slate-500"
                        >
                ...
                <div
                    class="hidden flex-col items-center justify-center gap-3 self-stretch border-x border-slate-100 px-1 py-4 text-slate-400 lg:flex"
                    aria-hidden="true"
                >
                ...
                <div class="min-w-0 space-y-2">
                    <p
                        class="text-xs text-blue-600"
                        x-cloak
                        :class="selectedVersionHead ? 'visible' : 'invisible'"
                    >
                        Selected:
                    ...
                    <div class="max-h-[min(420px,55vh)] overflow-y-auto overflow-x-hidden rounded-xl border border-slate-200 bg-white">
                        <template x-for="version in filteredVersionsHead" :key="'head-' + version.unique_key">
                            ...
                            >
                                <div class="flex min-w-0 items-start gap-3">
                                    <input
                                        type="radio"
                                        name="selected_version_head"
                                        class="mt-0.5 h-4 w-4 shrink-0 border-slate-300 text-blue-600 focus:ring-blue-500"
                                        :value="version.unique_key"
                                        x-model="selectedVersionHead"
                                    >
                                    <div class="min-w-0 space-y-0.5">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <h3 class="text-sm font-semibold text-slate-900" x-text="version.name"></h3>
                        ...
                        <div
                            x-show="filteredVersionsHead.length === 0"
                            class="px-4 py-8 text-sm text-slate-500"
                        >
                            No branches, tags, or releases match your filters.
                        </div>
        ...
        <div class="flex flex-col items-stretch gap-3 pt-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="order-1 flex justify-center sm:order-2 sm:flex-1">
                <x-ui.primary-button
                    type="button"
                    class="min-w-56"
                    @click="if (comparisonReady) currentStep = 3"
                >
