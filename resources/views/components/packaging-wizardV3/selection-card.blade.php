<x-ui.card class="p-8 w-full">
    <div class="space-y-6">
        ...
        {{-- ── GITHUB MODE: simple select from config ─────────────────────── --}}
        <div>
        ...
        {{-- ── GITLAB MODE: dynamic project browser ────────────────────────── --}}

        @if(!$gitlabConnected)
        <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-700 flex items-center gap-3">
            ...
            <span>GitLab is not connected. <a href="{{ route('gitlab.oauth.redirect') }}" class="underline font-semibold">Connect your GitLab account</a> on the Projects page first.</span>
        </div>
        @else
        ...
        <x-gitlab-project-browser 
            :selectable="true" 
            :scrollable="true" 
            projects-var="filteredGitlabProjects" 
            loading-var="gitlabLoading" 
            loading-explore-var="gitlabLoadingExplore" 
        />
        @endif {{-- gitlabConnected --}}
        @endif {{-- vcsProvider --}}
